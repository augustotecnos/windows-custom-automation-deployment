// =====================================================================================
// ARQUIVO: Fases/Fase7_Limpeza.cs
// FUNÇÃO: Gerencia todas as operações da Fase 7: Limpeza final do sistema e
//         preparação para entrega ao usuário.
//         - Remove o estado da automação do registro.
//         - Limpa pastas temporárias, como o cache de instaladores.
//         - Executa um script de finalização.
//         - Inicia o reboot final.
// =====================================================================================
...
namespace INSTALADOR_SOFTWARE_SE.Fases
{
    ...
    public class Fase7_Limpeza
    {
        ...
        private static readonly string CaminhoScriptFinalizacao = Path.Combine(AppConfig.DeploymentSharePath, "Scripts", "Finalize-System.ps1");
        ...
        public bool Executar()
        {
            ...
            try
            {
                ...
                // --- ETAPA 2: LIMPAR O CACHE DE INSTALADORES ---
                const string localCachePath = @"C:\SUPORTE"; // Apagamos a pasta raiz "SUPORTE"
                _logCallback($"Limpando pasta de cache de instalação ({localCachePath})...");
                if (Directory.Exists(localCachePath))
                {
                    try
                    {
                        Directory.Delete(localCachePath, true); // true para apagar recursivamente
                        _logCallback("Pasta de cache removida com sucesso.");
                    }
                    catch (Exception ex)
                    {
                        // Não paramos o processo por isso, apenas logamos como um aviso.
                        _logCallback($"AVISO: Não foi possível remover a pasta de cache. Verificação manual pode ser necessária. Erro: {ex.Message}");
                    }
                }
                else
                {
                    _logCallback("Pasta de cache não encontrada. Nenhuma ação necessária.");
                }

                // --- ETAPA 3: EXECUTAR O SCRIPT DE FINALIZAÇÃO DO SISTEMA ---
                _logCallback("Executando script para reativar UAC e outras configurações finais...");
                ...
                // --- ETAPA 4: MENSAGEM FINAL E REINICIALIZAÇÃO CONTROLADA ---
                _logCallback("------------------------------------------------------------------");
