...
4.  Edite o arquivo `AppConfig.cs` para apontar para o caminho do seu `DeploymentShare$` e então compile a aplicação C# em modo `Release`.
5.  Copie **todo o conteúdo da pasta `bin/Release`** da sua aplicação C# e o arquivo `network_config.json` para dentro da pasta `Support` no pendrive.
...
-   Edite o arquivo `AppConfig.cs` para definir o caminho do seu `DeploymentShare$` antes de compilar.
