...
.DESCRIPTION
    Esta função automatiza o processo de ingressar um computador no domínio 'gruposepromo.com.br'.
    Ela verifica a conectividade com o controlador de domínio, determina a Unidade Organizacional (OU) correta
...
.EXAMPLE
    PS C:\> Join-Domain -ComputerNameToJoin "PC-TI-01" -PerfilId "matriz" -SetorId "ti"

    Este comando ingressará o computador "PC-TI-01" na OU:
    "OU=COMPUTADORES,OU=TI,OU=LY MATRIZ,DC=gruposepromo,DC=com,DC=br"

.EXAMPLE
    PS C:\> Join-Domain -ComputerNameToJoin "PC-FILIAL-01" -PerfilId "filial" -SetorId "fortaleza"
...
.NOTES
    Autor: Seu Nome
    Data: 16/07/2024
    Versão: 2.0 - Atualizado para refletir a estrutura de OU de 'gruposepromo.com.br'.
#>
function Join-Domain {
    ...
    try {
        ...
        # Solicita as credenciais de administrador
        ...
        if (-not $credential) {
            throw "Operação cancelada pelo usuário. Nenhuma credencial foi fornecida."
        }
...
# --- COMO USAR A FUNÇÃO ---
# 1. Carregue esta função em sua sessão do PowerShell (copiando e colando ou usando dot-sourcing: . .\seu_script.ps1).
# 2. Chame a função com os parâmetros necessários.

# Exemplo de chamada para um PC da Matriz no setor de TI (descomente para testar):
Join-Domain -ComputerNameToJoin "NOME-DO-PC" -PerfilId "matriz" -SetorId "ti" -Verbose

# Exemplo de chamada para um PC da Filial de Fortaleza (descomente para testar):
# Join-Domain -ComputerNameToJoin "NOME-DO-PC" -PerfilId "filial" -SetorId "fortaleza" -Verbose
